<?php
$user = Auth::user();
$act_ships = $completed_shipment = $receivable = $earnings = 0;
foreach(DB::table('shipments')->where('driver_id', $user->id)->get() as $shipment){
  if($shipment->status == 0 || $shipment->status == 1 || $shipment->status == 2 || $shipment->status == 3){
    $act_ships ++;
    $receivable += $shipment->driver_commission;
  } else if($shipment->status == 4) {
    $completed_shipment ++;
    $earnings += $shipment->driver_commission;
  }
}
...
<?php
  ...
  for($rs = 5; $rs >= 0; $rs--){
    $revenue = DB::table('shipments')->where('driver_id', $user->id)->where('created_at', 'like', '%'.date('Y-m', strtotime('-'.$rs.' month')).'%')->where('status', 4)->sum('driver_commission');
    $total_revenue += $revenue;
    $revenues .= $revenue.', ';

    $receive = DB::table('shipments')->where('driver_id', $user->id)->where('created_at', 'like', '%'.date('Y-m', strtotime('-'.$rs.' month')).'%')->whereIn('status', [0,1,2,3])->sum('driver_commission');
    $total_receive += $receive;
    $receivables .= $receive.', ';

    $complete = count(DB::table('shipments')->where('driver_id', $user->id)->where('status', 4)->where('created_at', 'like', '%'.date('Y-m', strtotime('-'.$rs.' month')).'%')->get());
    $compcount += $complete;
    $completed .= $complete.', ';

    $init = count(DB::table('shipments')->where('driver_id', $user->id)->whereIn('status', [0,1,2,3])->where('created_at', 'like', '%'.date('Y-m', strtotime('-'.$rs.' month')).'%')->get());
    $initcount += $init;
    $initiated .= $init.', ';
          ...
          <div class="col-md-6">
            ...
            <div class="box box-primary">
              <div class="box-header with-border">
                <h3 class="box-title"><span class="text-aqua">Revenue (${{number_format($total_revenue, 2)}})</span></h3>
