<?php
$user = Auth::user();
$act_ships = $completed_shipment = $payable = $spents = 0;
foreach(DB::table('shipments')->where('shipper_id', $user->id)->get() as $shipment){
  if($shipment->status == 0 || $shipment->status == 1 || $shipment->status == 2 || $shipment->status == 3){
    $act_ships ++;
    $payable += $shipment->shipment_cost+$shipment->extra_charge;
  } else if($shipment->status == 4) {
    $completed_shipment ++;
    $spents += $shipment->shipment_cost+$shipment->extra_charge;
  }
}
      ...
      <div class="row">
        <div class="col-lg-3 col-xs-6">
          ...
          <div class="small-box bg-aqua">
            <div class="inner">
              <h3 title="Total Shipments">{{$act_ships+$completed_shipment}}</h3>
              <p>Active Shipments: {{$act_ships}}<br>Completed Shipments: {{$completed_shipment}}</p>
            </div>
            <div class="icon">
        ...
        <div class="col-lg-3 col-xs-6">
          ...
          <div class="small-box bg-red">
            <div class="inner">
              <h3 title="Total Revenue">${{number_format($payable + $spents, 2)}}</h3>
              <p>Total Spent: ${{number_format($spents, 2)}}<br>Accounts Payable: ${{number_format($payable, 2)}}</p>
            </div>
            <div class="icon">
...
<?php
  ...
  for($rs = 5; $rs >= 0; $rs--){
    $revenue = DB::table('shipments')->where('shipper_id', $user->id)->where('created_at', 'like', '%'.date('Y-m', strtotime('-'.$rs.' month')).'%')->where('status', 4)->sum('shipment_cost');
    $total_revenue += $revenue;
    $revenues .= $revenue.', ';

    $receive = DB::table('shipments')->where('shipper_id', $user->id)->where('created_at', 'like', '%'.date('Y-m', strtotime('-'.$rs.' month')).'%')->whereIn('status', [0,1,2,3])->get();
    // $receive->sum('shipment_cost');
    ...
    $complete = count(DB::table('shipments')->where('shipper_id', $user->id)->where('status', 4)->where('created_at', 'like', '%'.date('Y-m', strtotime('-'.$rs.' month')).'%')->get());
    $compcount += $complete;
    $completed .= $complete.', ';

    $init = count(DB::table('shipments')->where('shipper_id', $user->id)->whereIn('status', [0,1,2,3])->where('created_at', 'like', '%'.date('Y-m', strtotime('-'.$rs.' month')).'%')->get());
    $initcount += $init;
    $initiated .= $init.', ';
          ...
          <div class="col-md-6">
            ...
            <div class="box box-primary">
              <div class="box-header with-border">
                <h3 class="box-title"><span style="color:#666">Accounts Payable (${{number_format($total_receive, 2)}})</span> <span class="text-aqua">Spent (${{number_format($total_revenue, 2)}})</span></h3>
