          ...
          <div class="box">
            ...
            <div class="box-body table-responsive no-padding">
              <table class="table table-hover">
                ...
                <tr>
                  <td>{{App\User::find($user->refer_by)->first_name}}</td>
                  <td>{{$user->email}}</td>
                  <td>
                    @if($user->status == 1)
                    <span class="label-success"> &nbsp; Active &nbsp; </span>
                    @else
                    <span class="label-warning"> &nbsp; Yet to Sign Up &nbsp; </span>
                    @endif
                  </td>
                  <td>{{$user->sending_count}}/5</td>
                  <td>{{ date('d M Y', strtotime($user->created_at))}}</td>
                  <td>                    
                    {{-- <a href="#" class="label label-success" title="Login to this account"><i class="fa fa-search-plus"></i></a> --}}
                    {{-- <a href="/pending_referral/{{$user->id}}/details" title="Details" class="label label-info"><i class="fa fa-file-text"></i></a> --}}

                    <a href="/admin/resend_invitation/{{$user->id}}" class="label label-primary" onclick="return confirm('Are you sure you want to resend invitation to this email?')" title="Resend invitation."><i class="fa fa-envelope-o"></i></a>
